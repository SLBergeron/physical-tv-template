    ...
    <div className="relative flex p-8 gap-10 h-full">
      <div className=" w-full md:w-[90%] p-5  mx-auto bg-white dark:bg-neutral-900 shadow-2xl group h-full">
        <div className="flex flex-1 w-full h-full flex-col space-y-2 opacity-20 dark:opacity-60 ">
          <UserMessage>
            We&apos;re launching a fitness brand but the market is saturated. How do we stand out?
          </UserMessage>
          <AIMessage>
            Let&apos;s map your landscape. Who are you NOT competing with? What conversation are you joining?
          </AIMessage>
          <UserMessage>
            We focus on strength for everyday life, not aesthetics or performance.
          </UserMessage>
          <AIMessage>That&apos;s your distinction. Now, what&apos;s the framework that scales it?</AIMessage>
          <UserMessage>
            How do we position this across our content, product, and community?
          </UserMessage>
          <AIMessage>Strategy isn&apos;t a document - it&apos;s the alignment that runs through everything you build.</AIMessage>
        </div>
      </div>
...
const UserMessage = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    ...
    >
      <div className="h-4 w-4 md:h-10 md:w-10 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 flex-shrink-0" />
